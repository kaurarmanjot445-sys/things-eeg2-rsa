...
#Load ResNet hardcoded as best layer based on step 4 results(peak r=0.233 at 194)
resnet_rdm = np.load(os.path.join(RDM_DIR, "rdm_layer3.npy"))
print(f"  ResNet Layer 3 RDM shape: {resnet_rdm.shape}")
...
print(f"\nResNet50 Layer 3 peak: {resnet_peak_ms:.0f}ms  r={resnet_peak_r:.3f}")
...
ax.plot(TIMES_MS, rsa_resnet_s, color="#2ca02c", linewidth=2.8,
        label="ResNet50 Layer 3 (vision-only)")
